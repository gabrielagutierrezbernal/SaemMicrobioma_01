## ===== Benchmark ZIBBMR de las versiones HISTORICAS del paquete =====
## Mide los tiempos de v0 (sin optimizar) y Fase 1 (optimizacion en R puro) para
## el modelo de conteos, instalando cada commit en una libreria temporal y
## corriendo el ajuste en un proceso R SEPARADO (unica forma fiable: R no
## permite tener dos versiones del mismo paquete cargadas en una misma sesion).
##
...
##   setwd("ruta/al/repo"); source("estudios/comparacion_tiempos/benchmark_historico_zibbmr.R")
...
bench_data <- list(Y = datos$Y, S = datos$TotalCounts,
...
writeLines(sprintf('
  ...
  d <- readRDS("%s")
  f <- function() fit_zibbmr(y = d$Y, S = d$S, id = d$id, X = d$X, Z = d$Z,
...
cat("\n===== TIEMPOS HISTORICOS ZIBBMR (mediana, segundos) =====\n")
