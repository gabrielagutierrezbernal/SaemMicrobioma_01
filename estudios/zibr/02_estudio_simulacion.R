## ===== ESTUDIO DE SIMULACION - modelo ZIBR =====
## Repite el ajuste sobre muchos datasets simulados, para distintos tamanos de
## muestra, y muestra que al aumentar el numero de sujetos el PROMEDIO de los
## estimados se acerca al valor verdadero y su DISPERSION (error) se achica.
## Genera un grafico de cajas: estudio_simulacion_zibr.png
...
## --- Resumen 1: PROMEDIO de los estimados (se acerca a la verdad al subir n) ---
...
## --- Resumen 2: DESVIACION ESTANDAR (el error se achica al subir n) ---
cat("\n===== DESVIACION ESTANDAR de los estimados =====\n")
cat(sprintf("%-11s", "n_sujetos"), sprintf("%10s", names(verdadero)), "\n")
for (n_sub in sample_sizes)
...
## png(...) y dev.off().
png("estudio_simulacion_zibr.png", width = 1100, height = 700, res = 110)
...
par(op)
dev.off()
cat("\nGrafico guardado en: estudio_simulacion_zibr.png\n")
